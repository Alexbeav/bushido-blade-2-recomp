...
#
# Writes: dist/bushidoblade2-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix bushidoblade2 \
  --exe-name Bushido_Blade_2 \
  --omit-openbios \
  --display-name "Bushido Blade 2" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Bushido Blade 2 (USA) disc" \
  --runtime-dir mods \
